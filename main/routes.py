...
def result(post_id):
    ...
    return render_template('result.html', title='Результат', post_id=post_id, questions=questions, quest=quest,
...
def result_handling(post_id, score, user_id, test_id, passe_date):
    try:
        ...
        try:
            qq = PassedTests.query.filter_by(user_id=user_id, test_id=test_id).all()
            for q in qq:
                db.session.delete(q)
        ...
        flash(f'Тестирование сохранено!', 'success')
...
@app.route("/result_handling2/<int:test_id>",
           methods=['GET', 'POST'])
@login_required
def result_handling2(test_id):
    passed_test = db.session.query(PassedTests).filter(PassedTests.user_id == current_user.id
                                                       and PassedTests.test_id == test_id).order_by(
        PassedTests.id.desc()).first()
    temp_path = save_word(passed_test)
